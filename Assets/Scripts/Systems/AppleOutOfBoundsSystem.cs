...
public partial class AppleOutOfBoundsSystem : SystemBase
{
    ...
    protected override void OnCreate()
    {
        ...
        m_GameSettingsQuery = GetEntityQuery(typeof(GameSettingsComponent));
    }
    ...
    protected override void OnUpdate()
    {
        ...
        if (didMiss)
        {
            ...
            // destroy all apples
            foreach (var (_, apple) in SystemAPI.Query<RefRO<BadAppleTag>>().WithEntityAccess())
            {
                ecb.DestroyEntity(apple);
            }
            ...
            if (basketCount - 1 == 0)
            {
                if (!m_GameSettingsQuery.IsEmpty)
                {
                    ecb.DestroyEntity(m_GameSettingsQuery.GetSingletonEntity());
                }
